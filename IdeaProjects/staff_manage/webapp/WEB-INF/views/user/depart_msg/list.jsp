...
<meta name="description" content="部门">
			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					<div class="row">
						<div class="col-lg-2">
							<label>部门名</label>
							<input type="text" class="form-control" value="" placeholder="请输入部门名" id="dname" >
						</div>
						<div class="col-lg-1">
					...
					<div class="row"  style="margin-top:5px;padding-left: 15px;">
						<c:if test="${user.userType==1}">
							<a href="${pageContext.request.contextPath}/user/depart_msg/add" class="btn btn-primary">新增</a>
						</c:if>
					</div>
					...
					<div class="table-responsive">
						<table class="table  table-striped table-bordered  table-hover">
							...
							<tbody  id="depart_msg_body">
							</tbody>
						</table>
						<div id="depart_msg_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=0316"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/user/depart_msg/queryList",
		data : {
			"page":page,
			"dname":dname
		},
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				html += '<tr>' ;
				var dnameVal=setNullToEmpty(rows[i].departMsg.dname);
				html+='<td>'+ dnameVal+'</td>';
				html+='<td>';
				if('${user.userType}'=='1'){
					html+='<a href="${pageContext.request.contextPath}/user/depart_msg/update?id='+rows[i].departMsg.id+'" class="btn btn-success btnStyle">修改</a>';
				}
				if('${user.userType}'=='1'){
					html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].departMsg.id+'" class="btn btn-danger btnStyle">删除</a>';
				}
				html+='</td>';
				html+='</tr>';
			}
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/user/depart_msg/queryCount",
		async: false,
		data : {
			"dname":dname
		},
...
function del(e){
	if(window.confirm('你确定要删除该部门吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/user/depart_msg/del",
			data : {
				"id":id
			},
			success : function(result) {
