			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					<div class="row">
						<div class="col-lg-2">
							<label>职工号</label>
							<input type="text" class="form-control" value="" placeholder="请输入职工号" id="staffNo" >
						</div>
						<div class="col-lg-2">
							<label>姓名</label>
						...
						<div class="col-lg-2">
							<label>职位</label>
							<select  id="jobId" style="height:30px;width:200px"  >
								<option value="">全部</option>
					...
					<div class="row"  style="margin-top:5px;padding-left: 15px;">
						<c:if test="${user.userType==1}">
							<a href="${pageContext.request.contextPath}/user/staff_msg/add" class="btn btn-primary">新增</a>
						</c:if>
					</div>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/user/staff_msg/queryList",
		data : {
			"page":page,
			"staffNo":staffNo,
			"realName":realName,
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				var jobIdVal=setNullToEmpty(rows[i].jobIdStr);
				html+='<td>'+ jobIdVal+'</td>';
				var createDateVal=setNullToEmpty(rows[i].staffMsg.createDate);
				html+='<td>'+ createDateVal+'</td>';
				html+='<td>';
				if('${user.userType}'=='1'){
					html+='<a href="${pageContext.request.contextPath}/user/staff_msg/update?id='+rows[i].staffMsg.id+'" class="btn btn-success btnStyle">修改</a>';
				}
				if('${user.userType}'=='1'){
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/user/staff_msg/queryCount",
		async: false,
		data : {
			"staffNo":staffNo,
			"realName":realName,
...
function del(e){
	if(window.confirm('你确定要删除该员工吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/user/staff_msg/del",
			data : {
				"id":id
			},
			success : function(result) {
