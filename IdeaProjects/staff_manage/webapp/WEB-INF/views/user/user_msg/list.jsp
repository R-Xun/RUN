			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					...
					<div class="row"  style="margin-top:5px;padding-left: 15px;">
						<a href="${pageContext.request.contextPath}/user/user_msg/add" class="btn btn-primary">新增</a>
					</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=2688"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/user/user_msg/queryList",
		data : {
			"page":page,
			"loginName":loginName,
			"userType":userType
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>';
				html+='<a href="${pageContext.request.contextPath}/user/user_msg/update?id='+rows[i].userMsg.id+'" class="btn btn-success btnStyle">修改</a>';
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/user/user_msg/queryCount",
		async: false,
		data : {
			"loginName":loginName,
			"userType":userType
...
function del(e){
	if(window.confirm('你确定要删除该用户吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/user/user_msg/del",
			data : {
				"id":id
			},
			success : function(result) {
